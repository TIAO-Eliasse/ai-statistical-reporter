...
def display_profile_selector_ui():
    """Affiche un sélecteur de profil dans Streamlit"""
    import streamlit as st
    
    profiles = UserProfileManager.get_all_profiles()
    
    st.markdown("### 👤 Sélectionnez votre profil utilisateur")
    ...
    with col2:
        ...
        st.info(f"📝 {selected_profile['description']}")
    ...
    with st.expander("🔍 Voir les détails du profil"):
        config = manager.get_config()
        
        col_a, col_b = st.columns(2)
        
        with col_a:
            st.markdown("**Style rédactionnel**")
            st.write(f"• Ton : {config.tone}")
            st.write(f"• Niveau : {config.technical_level}")
            st.write(f"• Jargon : {'Oui' if config.jargon_allowed else 'Non'}")
        
        with col_b:
            st.markdown("**Analyses**")
            st.write(f"• Rigueur : {config.statistical_rigor}")
            st.write(f"• Tests : {'Oui' if config.hypothesis_testing else 'Non'}")
            st.write(f"• Prédiction : {'Oui' if config.predictive_modeling else 'Non'}")
...
if __name__ == "__main__":
    ...
    print("\n1️⃣ TOUS LES PROFILS DISPONIBLES :")
    profiles = UserProfileManager.get_all_profiles()
    for p in profiles:
        print(f"  {p['icon']} {p['name']} : {p['description']}")
    
    # Test 2 : Profil INDUSTRY
    print("\n2️⃣ TEST PROFIL INDUSTRY :")
    manager = UserProfileManager(UserProfile.INDUSTRY)
    print(manager.to_prompt_context())
    
    # Test 3 : Guidelines chapitre
    print("\n3️⃣ GUIDELINES POUR UN CHAPITRE :")
    guidelines = manager.get_chapter_guidelines("Analyse des corrélations")
    print(guidelines)
    
    # Test 4 : Validation
    print("\n4️⃣ VALIDATION STRUCTURE :")
    validation = manager.validate_chapter_structure("Revue de littérature")
    print(f"  Autorisé : {validation['allowed']}")
    if not validation['allowed']:
        print(f"  Raison : {validation['reason']}")
    
    # Test 5 : Changement profil
    print("\n5️⃣ CHANGEMENT VERS ACADEMIC :")
    manager.set_profile(UserProfile.ACADEMIC)
    validation2 = manager.validate_chapter_structure("Revue de littérature")
    print(f"  Autorisé : {validation2['allowed']}")
