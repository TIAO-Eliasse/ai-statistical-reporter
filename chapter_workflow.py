...
Génère un rapport complet chapitre par chapitre avec validation humaine
VERSION CORRIGÉE ET AMÉLIORÉE - Décembre 2025

CORRECTIONS MAJEURES :
[OK] Gestion robuste des erreurs Python (KeyError, TypeError)
[OK] Décodage des variables encodées
[OK] Interprétations obligatoires après CHAQUE tableau/graphique
[OK] Références correctes (tableau vs graphique)
...
# ═══════════════════════════════════════════════════════════════════════════════
# MODULE AUTO-REVIEW INTÉGRÉ v2.5 - Powered by Gemini 2.0 Flash
# ═══════════════════════════════════════════════════════════════════════════════

REVIEW_PROMPT = """Tu es un statisticien senior et reviewer professionnel.

[SEARCH] ANALYSE : Crédibilité statistique, qualité analytique, lisibilité professionnelle
🛠️ PRODUIS : 1️⃣ Critique 2️⃣ Améliorations 3️⃣ Recommandations prompts 4️⃣ Principes
...
def auto_review_report(report_markdown: str) -> dict:
    ...
    try:
        ...
        if not api_key:
            return {'success': False, 'error': 'GEMINI_API_KEY ou GOOGLE_API_KEY non trouvée'}
        ...
        logger.info("Auto-review terminée")
        
        # Parser sections
        sections = {}
        for emoji, key in [("1️⃣", "critique"), ("2️⃣", "ameliorations"), 
                            ("3️⃣", "prompt_recommendations"), ("4️⃣", "principles")]:
            if emoji in review_text:
                start = review_text.find(emoji)
                next_emoji = ["2️⃣", "3️⃣", "4️⃣", None]
                next_idx = ["2️⃣", "3️⃣", "4️⃣", None].index(None) if emoji == "4️⃣" else ["1️⃣", "2️⃣", "3️⃣", "4️⃣"].index(emoji) + 1
...
class ChapterStatus(Enum):
    ...
    GENERATED = "Généré (en attente validation)"
    VALIDATED = "Validé"
    REJECTED = "Rejeté (à regénérer)"
    ERROR = "Erreur"


class Chapter:
    """Représente un chapitre du rapport"""
...
class ReportGenerationWorkflow:
    """
    Workflow de génération de rapport avec validation humaine
    
    Process:
    1. Charger le plan du rapport
    2. Pour chaque chapitre:
       a. Générer le chapitre (IA + E2B)
       b. Afficher à l'utilisateur
       c. Attendre validation
       d. Si validé: passer au suivant
       e. Si rejeté: regénérer
    ...
    def __init__(self, user_id: str, plan: Dict, csv_path: str, cost_controller=None, study_context=None):
        ...
        # Charger la mémoire contextuelle
    ...
    def generate_current_chapter(self) -> Dict:
        ...
        if not chapter:
            return {
                'success': False,
                'chapter': None,
                'error': "Aucun chapitre à générer"
            }
    ...
    def _detect_task_type(self, chapter: Chapter) -> str:
        """
        Détecte le type de tâche selon le numéro et titre du chapitre
        ...
        # Chapitres nécessitant beaucoup de code/graphiques
        if any(keyword in title_lower for keyword in [
            "présentation", "description", "distribution", "visualisation"
        ]):
            return "code"
        
        # Chapitres d'analyse/interprétation
        elif any(keyword in title_lower for keyword in [
            "analyse", "relation", "corrélation", "test", "interprétation", 
            "comparaison", "différence"
        ]):
            return "analysis"
        
        # Chapitres de conclusion/synthèse
        elif any(keyword in title_lower for keyword in [
            "conclusion", "synthèse", "recommandation", "discussion"
        ]):
            return "writing"
    ...
    def _build_chapter_prompt(self, chapter: Chapter, context: str) -> str:
        """
        Construit le prompt pour générer un chapitre
        VERSION ULTRA-ORGANISÉE - Structure hiérarchique claire
        """
        ...
        try:
            ...
            csv_columns_info = f"""
    📁 FICHIER {file_type_label} : {len(df_temp)} lignes
    ...
    """
            for col in columns_list[:20]:  # Limiter affichage
                ...
                csv_columns_info += f"  • \"{col}\" ({dtype}, ex: {example})\n"
            
            if len(columns_list) > 20:
                csv_columns_info += f"  • ... et {len(columns_list) - 20} autres colonnes\n"
            
        except Exception as e:
            logger.error(f"Failed to read data: {e}")
            csv_columns_info = "[WARNING] Impossible de lire les colonnes du fichier.\n"
        
        # Récupérer contraintes de longueur
        length_guidelines = ""
        if self.cost_controller:
            try:
                ...
                if config:
                    length_guidelines = f"\n📏 LONGUEUR CIBLE : {config.target_pages} pages (~{config.target_words} mots)\n"
            except Exception as e:
        ...
        - Répondez à la question de recherche
        - Testez les hypothèses formulées
        - Atteignez les objectifs définis
        - Adaptez le style au public cible

        """
                
                # NOUVEAU : Bloc de style d'écriture selon le profil
                try:
    ...
    [REFRESH] RÉGÉNÉRATION DEMANDÉE : {instructions.get('request', '')}
    {"[OK] Conserver structure" if instructions.get('keep_structure') else "🆕 Modifier si nécessaire"}
    """
        elif chapter.status == ChapterStatus.REJECTED and chapter.error_message:
            regeneration_instructions = f"\n[REFRESH] RAISON DU REJET : {chapter.error_message}\n"
        
        # ═══════════════════════════════════════════════════════════════
        # CONSTRUCTION DU PROMPT HIÉRARCHISÉ
        # ═══════════════════════════════════════════════════════════════
    ...
    ╔══════════════════════════════════════════════════════════════════════════════╗
    ║ SECTION 1 : DONNÉES & CONTEXTE DISPONIBLES                                  ║
    ╚══════════════════════════════════════════════════════════════════════════════╝
    ...
    ╔══════════════════════════════════════════════════════════════════════════════╗
    ║ SECTION 2 : VOTRE MISSION                                                   ║
    ╚══════════════════════════════════════════════════════════════════════════════╝

    [WARNING] RÈGLE ULTRA-IMPORTANTE : LE CODE NE DOIT PAS APPARAÎTRE DANS LE RAPPORT !

    🔴 FONCTIONNEMENT DU SYSTÈME :
    1. Tu écris le code entre ```python et ```
    2. Le système l'EXÉCUTE automatiquement dans E2B
    3. Le CODE est SUPPRIMÉ du rapport final
    4. Seuls les RÉSULTATS (tableaux, graphiques, print) apparaissent
    🔴 PRINCIPE DE NON-RÉPÉTITION :
    - Une fois qu'un résultat est affiché (tableau/graphique), NE PAS le régénérer
    - Si vous devez ajuster, utilisez les données déjà calculées
    - Exemple :
...
````
    ...
    [Code 2 → Tableau 1.2 - NOUVEAU résultat]
    afficher tableau
    Interprétation...
    [Code 3 → graphique 1.1 - NOUVEAU résultat]
...
````
    [Code 1 → Tableau 1.1]
    [Code 1 BIS (même chose) → Tableau 1.1 dupliqué]
    le code ne doit jamais etre afficher dans le rapport
````
    [ERROR] CE QUI APPARAÎT ACTUELLEMENT (ERREUR) :
    le code ne doit jamais etre afficher dans le rapport
    ```
    ### 2.1. Analyse
    ...
    **Tableau 2.1 : Résultats**
    ...
    [OK] CE QUI DOIT APPARAÎTRE (CORRECT) :
    ```
    ### 2.1. Analyse
    ...
    **Tableau 2.1 : Résultats**
    ...
    🔴 DONC :
    - N'écris JAMAIS "Voici le code Python..." ou "Le code suivant..."
    - N'explique PAS ton code avant de l'écrire
    - Écris DIRECTEMENT le code, il sera exécuté et supprimé
    - Seuls les print() et plt.show() généreront du contenu visible

    📋 SECTIONS À COUVRIR :

    """
        
        # Ajouter sections à couvrir
    ...
    - Chiffres précis + interprétation


    ╔══════════════════════════════════════════════════════════════════════════════╗
    ║ SECTION 3 : TOP 10 RÈGLES CRITIQUES (PAR ORDRE D'IMPORTANCE)                ║
    ╚══════════════════════════════════════════════════════════════════════════════╝

    Lisez TOUTES ces règles avant de commencer. Elles sont classées par importance.
    ┌────────────────────────────────────────────────────────────────────────┐
    │ 🔴 RÈGLE #0 : CODE = INVISIBLE (LA RÈGLE SUPRÊME)                      │
    └────────────────────────────────────────────────────────────────────────┘
    ...
    [ERROR] CE QUI NE DOIT JAMAIS APPARAÎTRE DANS LE RAPPORT :
````python
    import pandas as pd
    df = pd.read_excel(...)
    # ... tout code Python ...
````
    ...
    [OK] CE QUI DOIT APPARAÎTRE :
    - Les tableaux de résultats
    - Les graphiques
    - Les interprétations en prose
    - Les insights business

    🔧 COMMENT ÇA MARCHE :
    1. Vous écrivez le code entre ```python et ```
    2. Le système l'EXÉCUTE dans un sandbox
    3. Le système SUPPRIME le code du rapport
    4. Le système GARDE uniquement les résultats (print, graphiques)

    [WARNING] DONC :
    - N'expliquez JAMAIS votre code au lecteur
    - Ne dites JAMAIS "Voici le code..." ou "Le code suivant..."
    - Écrivez comme si le code n'existait pas pour le lecteur
    - Seuls les RÉSULTATS comptent

    📖 STYLE D'ÉCRITURE CORRECT :
````
    L'analyse de la structure de financement révèle des insights clés.

    **Tableau 1.1 : Corrélation Fonds Propres - CA 2015**
    
    | Variable 1 | Variable 2 | Corrélation |
    |------------|------------|-------------|
    | Fonds propres | CA 2015 | 0.04 |

    Cette faible corrélation (0.04) suggère que...
````

    [ERROR] CE QU'IL NE FAUT PAS ÉCRIRE :
    ┌──────────────────────────────────────────────────────────────────────────────┐
    │ 🔴 RÈGLE #1 : INITIALISATION OBLIGATOIRE (LA PLUS IMPORTANTE)               │
    └──────────────────────────────────────────────────────────────────────────────┘
    ...
    [ERROR] SANS CE BLOC → NameError: name 'chapitre' is not defined
    [OK] TOUJOURS copier ce bloc au début de CHAQUE code
    ┌────────────────────────────────────────────────────────────────────────┐
    │ 🔴 RÈGLE #1.5 : PAS DE CONFIGURATION E2B DANS LE CODE                  │
    └────────────────────────────────────────────────────────────────────────┘

    [WARNING] RÈGLE CRITIQUE : FORMAT DU CODE PYTHON
...
````
    ...
    - NE JAMAIS écrire: import os; os.environ[...]
    - NE JAMAIS ajouter de configuration d'environnement
    - NE JAMAIS mettre de code avant les imports

    Le système gère automatiquement l'environnement d'exécution.
    ┌──────────────────────────────────────────────────────────────────────────────┐
    │ 🔴 RÈGLE #2 : INTERPRÉTATION IMMÉDIATE (CAUSE #1 DE REJET)                  │
    └──────────────────────────────────────────────────────────────────────────────┘

    ORDRE STRICT pour chaque analyse :
    1️⃣ Introduction (1-2 phrases)
    2️⃣ Code Python → Résultat
    3️⃣ Interprétation (2-4 phrases en prose)
    4️⃣ Transition (1 phrase)

    [ERROR] INTERDIT : 2+ tableaux/graphiques sans interprétation entre eux
    [OK] OBLIGATOIRE : 1 élément = 1 interprétation IMMÉDIATE

    EXEMPLE CORRECT :
    ```
    Examinons la répartition géographique.

    [Code → Tableau]

    Les données révèlent une concentration dans le LITTORAL (37%)...
    Cette prédominance reflète le dynamisme économique côtier.
    ...
    ┌──────────────────────────────────────────────────────────────────────────────┐
    │ 🔴 RÈGLE #3 : GUILLEMETS DANS LES F-STRINGS (CAUSE #1 D'ERREURS)            │
    └──────────────────────────────────────────────────────────────────────────────┘
    ...
    [OK] SOLUTION :
    print(f"Analyse 'Chiffre d\\'affaire'")  # Guillemets simples + échappement

    📌 RÈGLE : F-string = " extérieur, ' intérieur

    ┌──────────────────────────────────────────────────────────────────────────────┐
    │ 🔴 RÈGLE #4 : TRY-EXCEPT OBLIGATOIRE (ZÉRO ERREUR DANS RAPPORT)             │
    └──────────────────────────────────────────────────────────────────────────────┘

    TOUT code graphique DOIT être dans try-except :
    ...
    try:
        ...
        # Fallback tableau
        print(f"**Tableau {{chapitre}}.{{tableau_counter}} : Données**")
        # ... tableau simple ...
        tableau_counter += 1
    ```
    ...
    ┌──────────────────────────────────────────────────────────────────────────────┐
    │ 🔴 RÈGLE #5 : VARIABLES ENCODÉES (ERREUR FRÉQUENTE)                         │
    └──────────────────────────────────────────────────────────────────────────────┘

    AVANT calcul moyenne, VÉRIFIER :

    ```python
    if df['colonne'].nunique() < 10:
        # Encodée → distribution seulement
        print("Distribution :")
        # ... fréquences ...
    else:
        # Continue → statistiques
        print(f"Moyenne : {{df['colonne'].mean():.2f}}")
    ```

    [ERROR] ERREUR : Moyenne sur Age encodé (1,2,3) → "2.5 ans" absurde !
    [OK] CORRECT : Vérifier d'abord

    ┌──────────────────────────────────────────────────────────────────────────────┐
    │ 🔴 RÈGLE #6 : NUMÉROTATION SYSTÉMATIQUE                                     │
    └──────────────────────────────────────────────────────────────────────────────┘
    ...
    [ERROR] INTERDIT : Oublier numérotation
    [OK] OBLIGATOIRE : Numéro séquentiel

    ┌──────────────────────────────────────────────────────────────────────────────┐
    │ 🔴 RÈGLE #7 : ÉQUILIBRE TABLEAUX/GRAPHIQUES (40% GRAPHIQUES MIN)            │
    └──────────────────────────────────────────────────────────────────────────────┘

    CONVERSIONS OBLIGATOIRES :
    - Distribution (>5 cat) → GRAPHIQUE barres
    - Corrélations → HEATMAP
    - Moyennes/groupe → GRAPHIQUE barres
    - Évolutions → GRAPHIQUE ligne

    [ERROR] INTERDIT : Chapitre avec que des tableaux
    [OK] OBJECTIF : 40% minimum de graphiques

    ┌──────────────────────────────────────────────────────────────────────────────┐
    │ 🔴 RÈGLE #8 : APOSTROPHES & CARACTÈRES SPÉCIAUX                             │
    └──────────────────────────────────────────────────────────────────────────────┘

    Colonnes avec apostrophes :

    [ERROR] ERREUR : cols = ['Nombre d'année']  # SyntaxError
    [OK] SOLUTION 1 : cols = ["Nombre d'année"]  # Guillemets doubles
    [OK] SOLUTION 2 : cols = ['Nombre d\\'année']  # Échapper

    ┌──────────────────────────────────────────────────────────────────────────────┐
    │ 🔴 RÈGLE #9 : CONVERSION FLOAT AVANT FORMATAGE                              │
    └──────────────────────────────────────────────────────────────────────────────┘

    TOUJOURS convertir Pandas → float :
    ...
    ┌──────────────────────────────────────────────────────────────────────────────┐
    │ 🔴 RÈGLE #10 : ORDRE STRICT GRAPHIQUES                                      │
    └──────────────────────────────────────────────────────────────────────────────┘

    ORDRE NON-NÉGOCIABLE :
    1️⃣ Code Python
    2️⃣ plt.show()
    3️⃣ print(f"**Graphique...**")  ← APRÈS plt.show() !
    4️⃣ Interprétation

    [ERROR] INTERDIT : Titre avant plt.show()
    [OK] OBLIGATOIRE : Titre TOUJOURS après plt.show()

    ┌──────────────────────────────────────────────────────────────────────────────┐
    │ 🔴 RÈGLE #11 : JAMAIS DE TABLEAUX CONSÉCUTIFS (ULTRA-CRITIQUE)              │
    └──────────────────────────────────────────────────────────────────────────────┘

    [ERROR] INTERDIT ABSOLU :
    ```
    Tableau 2.1 : Stats Age
    Tableau 2.2 : Stats Diplôme  ← SANS interprétation du 2.1 !
    ```
    ...
    [INTERPRÉTATION IMMÉDIATE - minimum 2 phrases]
    L'analyse révèle que... Les données montrent...

    Tableau 2.2 : Stats Diplôme

    [INTERPRÉTATION IMMÉDIATE - minimum 2 phrases]
    On observe que... Cette distribution suggère...
    ```

    📌 RÈGLE D'OR : 1 tableau = 1 interprétation AVANT le tableau suivant

    ┌──────────────────────────────────────────────────────────────────────────────┐
    │ 🔴 RÈGLE #12 : VALEURS MANQUANTES OBLIGATOIRES (SI > 0%)                    │
    └──────────────────────────────────────────────────────────────────────────────┘
    Tous les codes sont des prompt que vous devez adaptez pour éviter des erreurs comme count is not define ou encore missing_count is not define
    SI des valeurs manquantes existent, OBLIGATOIRE :

    1️⃣ CRÉER une section dédiée : "Qualité des données"

    2️⃣ ANALYSER CHAQUE variable avec valeurs manquantes :
    ...
    [OK] OBLIGATOIRE : Section complète si valeurs manquantes > 0%


    ╔══════════════════════════════════════════════════════════════════════════════╗
    ║ SECTION 4 : TEMPLATES CODE PRÊTS À L'EMPLOI                                 ║
    ╚══════════════════════════════════════════════════════════════════════════════╝

    Copiez-collez ces templates selon vos besoins.

    ┌──────────────────────────────────────────────────────────────────────────────┐
    │ 📝 TEMPLATE 1 : Distribution avec fallback                                  │
    └──────────────────────────────────────────────────────────────────────────────┘
    ...
    ┌──────────────────────────────────────────────────────────────────────────────┐
    │ 📝 TEMPLATE 2 : Corrélations (heatmap)                                      │
    └──────────────────────────────────────────────────────────────────────────────┘
    ...
    try:
        ...
        print(f"**Graphique {{chapitre}}.{{figure_counter}} : Corrélations**")
        figure_counter += 1

    except:
        print(f"**Tableau {{chapitre}}.{{tableau_counter}} : Corrélations**")
        # ... tableau corrélations ...
        tableau_counter += 1
    ```

    ┌──────────────────────────────────────────────────────────────────────────────┐
    │ 📝 TEMPLATE 3 : Variable encodée                                            │
    └──────────────────────────────────────────────────────────────────────────────┘
    ...
    ┌──────────────────────────────────────────────────────────────────────────────┐
    │ 📝 TEMPLATE 4 : Analyse valeurs manquantes (OBLIGATOIRE si NaN > 0)         │
    └──────────────────────────────────────────────────────────────────────────────┘

    ```python
    # Identifier valeurs manquantes
    Exemple de code à adapter selon le contexte :
    missing = df.isnull().sum()
    missing = missing[missing > 0].sort_values(ascending=False)
    ...
    if len(missing) > 0:
        ...
        for col, count in missing.items():
            pct = count / len(df) * 100
            impact = "Faible" if pct < 1 else "Modéré" if pct < 5 else "Élevé"
        ...
        print(f\"\"\"L'analyse des valeurs manquantes révèle que {{len(missing)}} variables 
    sont concernées. La variable '{{missing.index[0]}}' présente le taux le plus 
    élevé avec {{missing.iloc[0] / len(df) * 100:.2f}}% de données manquantes. 
    Ces valeurs manquantes nécessitent un traitement approprié selon leur nature 
    et leur proportion avant toute modélisation.\"\"\")
    ```


    ╔══════════════════════════════════════════════════════════════════════════════╗
    ║ SECTION 5 : CHECKLIST AVANT ENVOI                                           ║
    ╚══════════════════════════════════════════════════════════════════════════════╝

    Avant de finaliser, VÉRIFIEZ :

    ☐ Initialisation (chapitre, counters) au début de CHAQUE code
    ☐ Interprétation IMMÉDIATE après CHAQUE tableau/graphique
    ☐ JAMAIS 2 tableaux ou 2 graphiques consécutifs sans interprétation
    ☐ Guillemets simples ' dans f-strings (pas ")
    ☐ Try-except autour de TOUS les graphiques
    ☐ Variables encodées vérifiées avant moyenne
    ☐ Numérotation séquentielle tableaux/graphiques
    ☐ 40% minimum de graphiques
    ☐ Apostrophes échappées
    ☐ Conversion float avant formatage
    ☐ plt.show() AVANT print("**Graphique...**")
    ☐ Section "Valeurs Manquantes" si des NaN existent
    ☐ Pas de duplication de code ou de tableaux
    ☐ Chaque analyse est unique et progressive
    ☐ Si un résultat est déjà montré, passer à l'analyse suivante



    ╔══════════════════════════════════════════════════════════════════════════════╗
    ║ 🎯 RÈGLES ADDITIONNELLES IMPORTANTES                                        ║
    ╚══════════════════════════════════════════════════════════════════════════════╝

    📌 SÉLECTION VARIABLES :
    - Max 10 pour stats descriptives
    - Max 5 pour corrélations
    ...
    📌 LABELS VARIABLES ENCODÉES :
    - Pas "Tranche 1, Tranche 2"
    - Utiliser labels contextuels (ex: "< 25 ans", "25-35 ans")

    📌 INTERDICTIONS ABSOLUES :
    - [ERROR] print(df.info())
    - [ERROR] df.to_markdown() ou .to_markdown() (utiliser .to_string() à la place)
    - [ERROR] pd.read_csv() ou pd.read_excel() (df déjà chargé automatiquement)
    - [ERROR] import os; os.environ[...] (configuration système interdite)
    - [ERROR] Statsmodels (utiliser scipy.stats)
    - [ERROR] Créer des variables intermédiaires (df_kpi, df_temp, etc.) utilisées dans un autre bloc
    - [ERROR] Blocs try: vides (toujours mettre au moins 'pass' dans le try)
    - [ERROR] Paramètres matplotlib abrégés (ha=, va=) - utiliser noms complets
    - [ERROR] Plusieurs éléments visuels sans interprétation
    - [ERROR] Tableaux consécutifs sans interprétation entre eux
    - [ERROR] Mentionner valeurs manquantes sans les analyser
    - [ERROR] Écrire "Voici le code..." ou expliquer le code avant de l'écrire
    - [ERROR] Dire "Le code suivant..." ou "Nous allons exécuter..."
    📌 COMMENT ÉCRIRE :
    [ERROR] MAUVAIS :

    ```
    Voici le code Python pour analyser les données :
    ...
    [OK] CORRECT :
    ```
    Analysons la distribution des entreprises par région.

    ```python
    # Pour afficher un tableau Markdown
    print(f"**Tableau {{chapitre}}.{{tableau_counter}} : Distribution par région**")
    print()
    print("| Région | Nombre | % |")
    print("|--------|--------|---|")
    ...
    L'analyse révèle une forte concentration dans la région OUEST...
    ```
    ...
    [OK] UTILISER À LA PLACE :
    ```python
    # Méthode 1: Tableaux manuels (recommandé pour contrôle)
    print("| Col1 | Col2 |")
    print("|------|------|")
    print(f"| {{val1}} | {{val2}} |")
    
    # Méthode 2: to_string() pour tableaux simples
    print(df.head().to_string())
    ```

    → Le code sera EXÉCUTÉ et SUPPRIMÉ, seuls les résultats restent !

    [WARNING] RÈGLE CRITIQUE : INDÉPENDANCE DES BLOCS DE CODE
    
    Chaque bloc ```python...``` est exécuté INDÉPENDAMMENT.
    Les variables créées dans un bloc N'EXISTENT PAS dans le bloc suivant !
    ...
    ```python
    # Créer, calculer ET utiliser dans le MÊME bloc
    ca_per_emp = df['CA'] / df['Effectifs'].replace(0, 1)  # Éviter division par 0
    result = ca_per_emp.groupby(df['Region']).mean()
    
    print("**Tableau X : CA par employé par région**")
    ...
    - NE créez PAS df_kpi, df_temp, df_filtered, etc.
    - Faites tous les calculs dans UN SEUL bloc de code

    [DATA] RÈGLES MATPLOTLIB :
    
    [ERROR] INTERDIT (paramètres abrégés) :
    ```python
    ...
    ```python
    plt.xticks(rotation=45, ha='center')  # [OK]
    ...
    📌 OBLIGATIONS :
    - Interprétation APRÈS CHAQUE tableau/graphique
    - Section "Valeurs Manquantes" si NaN > 0%
    - Minimum 2 phrases par interprétation
    - Références correctes (tableau vs graphique)

    Autres promptes a prendre en compte

    [OK] EXEMPLE  : Graphiques avec détection automatique
    ...
    # Créer un histogramme avec la première colonne numérique (non encodée)
    if numeric_cols:
        # Trouver première colonne avec > 10 valeurs uniques
    ...
    1️⃣ Introduction contextuelle (1-2 phrases)
    2️⃣ [Code Python → Élément visuel (tableau OU graphique)]
    3️⃣ Interprétation immédiate en prose (2-4 phrases, SANS marqueur)
    4️⃣ Implications/discussion (1-2 phrases)
    5️⃣ Transition vers l'analyse suivante (1 phrase)
    6️⃣ [Répéter pour chaque élément visuel]

    🔴 CARACTÉRISTIQUES D'UN RAPPORT PROFESSIONNEL :

    [OK] CE QU'ON VEUT :
    - Prose fluide et naturelle
    - Transitions entre les parties
    - Interprétation intégrée au texte
    - Ton analytique mais accessible
    - Style de publication scientifique
    - JAMAIS 2+ tableaux/graphiques sans interprétation entre eux

    [ERROR] CE QU'ON NE VEUT PAS :
    - **Interprétation :** ...
    - **Analyse :** ...
    - **Discussion :** ...
    - Bullet points pour les interprétations
    - Style "devoir scolaire"
    - Plusieurs éléments visuels consécutifs sans texte

    ═══════════════════════════════════════════════════════════════════════════════
    7. RÈGLES DE CONCISION
    ═══════════════════════════════════════════════════════════════════════════════

    [WARNING] ÉVITER LA VERBOSITÉ EXCESSIVE :

    [ERROR] TROP LONG :
    "L'analyse approfondie des données collectées révèle de manière significative et 
    notable que la distribution observée des valeurs au sein de l'échantillon étudié 
    présente une configuration particulière qui mérite une attention toute particulière..."

    [OK] CONCIS ET PRÉCIS :
    "L'analyse révèle une distribution asymétrique, avec 75% des valeurs concentrées 
    entre 20 et 40 ans. Cette concentration suggère une population jeune, cohérente 
    avec le contexte urbain."

    RÈGLES :
    - Phrases de 15-25 mots maximum
    - Éviter les répétitions ("significatif", "notable", "important")
    - Aller droit au fait
    - Chiffres précis plutôt que qualificatifs vagues
    ...
    - Vérifier si variables encodées avant calculs de moyennes/écarts-types

    ═══════════════════════════════════════════════════════════════════════════════
    9. À ÉVITER ABSOLUMENT
    ═══════════════════════════════════════════════════════════════════════════════

    [ERROR] Répéter les analyses des chapitres précédents
    [ERROR] Utiliser les mêmes graphiques qu'avant
    [ERROR] Être trop technique sans vulgariser
    [ERROR] Enchaîner 2+ tableaux/graphiques sans interprétation entre eux
    [ERROR] Dire "graphique ci-dessus" quand c'est un tableau
    [ERROR] Dire "tableau ci-dessus" quand c'est un graphique
    [ERROR] Utiliser des marqueurs scolaires (**Interprétation:**, **Analyse:**)
    [ERROR] Calculer des moyennes sur des variables encodées

    ═══════════════════════════════════════════════════════════════════════════════

    DONNÉES :
    Le fichier de données est disponible à : {{self.csv_path}}

    Génère maintenant le Chapitre {chapter.number} en Markdown complet en respectant 
    SCRUPULEUSEMENT toutes ces règles, en particulier :
    1. Interprétation IMMÉDIATE après CHAQUE tableau/graphique
    2. Vérification des variables encodées
    3. Références correctes (tableau vs graphique)
    4. Gestion robuste des erreurs (KeyError, TypeError)
    5. Conversion float() avant formatage
    ══════════════════════════════════════════════════════════════
    🔴 Rappel:RÈGLE D’ORDRE ABSOLUE DES GRAPHIQUES (NON NÉGOCIABLE)
    ══════════════════════════════════════════════════════════════

    POUR CHAQUE GRAPHIQUE, L’ORDRE DE SORTIE DOIT ÊTRE STRICTEMENT :

    1️⃣ CODE PYTHON AVEC plt.show()  
    2️⃣ LIGNE DE TITRE DU GRAPHIQUE (Graphique X.Y : …)  
    3️⃣ PARAGRAPHE D’INTERPRÉTATION IMMÉDIAT (2–4 phrases)

    [WARNING] INTERDICTIONS FORMELLES :
    [ERROR] Ne JAMAIS écrire le titre du graphique AVANT le code
    [ERROR] Ne JAMAIS écrire l’interprétation AVANT plt.show()
    [ERROR] Ne JAMAIS écrire “Le graphique ci-dessus…” AVANT que le graphique soit affiché

    [OK] Le titre du graphique DOIT être imprimé JUSTE APRÈS plt.show() via print().

    ```markdown
    🔴 RÈGLE LEXICALE STRICTE :

    L’expression “Le graphique ci-dessus” est AUTORISÉE UNIQUEMENT :
    [OK] après que le graphique a été affiché (plt.show())
    [OK] après que la ligne “Graphique X.Y : …” a été imprimée

    [ERROR] INTERDIT d’utiliser cette expression avant le code ou avant le titre.```

    ╔══════════════════════════════════════════════════════════════════════════════╗
    ║ 🚀 GÉNÉREZ MAINTENANT LE CHAPITRE !                                         ║
    ╚══════════════════════════════════════════════════════════════════════════════╝

    Données : {self.csv_path}

    Suivez scrupuleusement les 10 RÈGLES CRITIQUES.
    ...
    def _get_available_variables(self) -> str:
        ...
        try:
            ...
            return "(colonnes non listées)"
    
    def _generate_with_ai(self, prompt: str, task_type: str = "default") -> str:
        """
        Génère le contenu avec l'IA (Gemini ou Claude en backup)
        
        Args:
            prompt: Le prompt
            task_type: Type de tâche pour sélectionner le meilleur modèle
        
        NOUVEAUTÉ: Support multi-modèles + fallback automatique + spécialisation
        """
        import os
        
        # Vérifier si on force l'utilisation de Claude
        ...
        # Essayer Gemini d'abord avec le modèle spécialisé
        try:
            ...
            # Si quota dépassé, essayer Claude en backup
    ...
    def _generate_with_gemini(self, prompt: str, task_type: str = "default") -> str:
        """
        Génère avec Gemini en utilisant un modèle spécialisé selon la tâche
        
        Args:
            prompt: Le prompt à envoyer
            task_type: Type de tâche ("code", "writing", "analysis", "plan", "default")
        """
        import google.generativeai as genai
        import os
    ...
    def _generate_with_claude(self, prompt: str) -> str:
        """Génère avec Claude (backup si Gemini quota dépassé)"""
        import os
    ...
    def _execute_code_blocks(self, content: str) -> str:
            """
            Exécute les blocs de code Python. 
            Version stabilisée : respecte l'indentation interne des blocs try/if.
            """
            import re
            import os
            import base64
            ...
            for match in reversed(matches):
                ...
                # sans toucher à l'indentation relative des blocs internes (try/except/if)
                ...
                # Correction 1: Compléter les blocs try vides
                ...
                # Correction 2: Remplacer variables non définies par 'df'
                ...
                # Correction 3: Remplacer paramètres matplotlib abrégés
                #if 'ha=' in final_code:
...
"""
    ...
    def _format_output_as_markdown(self, text: str) -> str:
        """Convertit les sorties texte en Markdown formaté avec tableaux professionnels"""
    ...
    def _convert_stats_to_markdown_table(self, stats_text: str) -> str:
        ...
        try:
            ...
            try:
                ...
                # Si le parsing échoue, formater manuellement
    ...
    def validate_chapter(self, chapter_number: str) -> bool:
        ...
        Returns:
            True si chapitre trouvé et validé
        """
    ...
    def reject_chapter(self, chapter_number: str, reason: str = None) -> bool:
        """
        Rejette un chapitre (à regénérer)
        
        Returns:
            True si chapitre trouvé et rejeté
        """
        chapter = self._find_chapter(chapter_number)
    ...
    def _find_chapter(self, chapter_number: str) -> Optional[Chapter]:
        """Trouve un chapitre par son numéro"""
    ...
    def is_complete(self) -> bool:
        """Vérifie si tous les chapitres sont validés"""
        return all(
            chapter.status == ChapterStatus.VALIDATED
            for chapter in self.chapters
        )
    ...
    def auto_review_final_report(self, report_markdown: str = None, save_to_file: bool = True) -> dict:
        ...
        Args:
            ...
            save_to_file: Sauvegarde rapport d'amélioration
        """
        logger.info("=" * 70)
    ...
    def compile_report(self) -> str:
        """
        Compile tous les chapitres validés en un rapport final
...
def initialize_workflow(user_id: str, plan: Dict, csv_path: str, cost_controller=None, study_context=None) -> ReportGenerationWorkflow:
    """
    Initialise un workflow de génération
    
    Args:
        user_id: ID de l'utilisateur
        plan: Plan du rapport
        csv_path: Chemin vers le fichier CSV/Excel
        cost_controller: (Optionnel) Contrôleur de coûts
        study_context: (Optionnel) Contexte de l'étude
...
def display_workflow_progress(workflow: ReportGenerationWorkflow):
    ...
    st.progress(
        ...
        text=f"Progression : {progress['validated_chapters']}/{progress['total_chapters']} chapitres validés"
    )
    ...
    for i, chapter in enumerate(workflow.chapters, 1):
        status_emoji = {
            ...
            ChapterStatus.GENERATING: "▶️",
            ChapterStatus.GENERATED: "👁️",
            ChapterStatus.VALIDATED: "[OK]",
            ChapterStatus.REJECTED: "[ERROR]",
            ChapterStatus.ERROR: "[WARNING]"
        }
        
        emoji = status_emoji.get(chapter.status, "•")
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("TEST REPORT GENERATION WORKFLOW - VERSION CORRIGÉE")
    print("="*60)
        ...
        'chapitres': [
            {
                ...
                'sections': [
                    {
                        'titre': 'Contexte',
                        'analyses': ['Présentation données', 'Objectifs']
                    }
                ]
            },
            {
                ...
                'sections': [
                    {
                        'titre': 'Statistiques',
                        'analyses': ['Moyennes', 'Écarts-types']
                    }
                ]
            }
        ]
    ...
    print(f"[OK] {len(workflow.chapters)} chapitres initialisés")
    ...
    print("\n[OK] Tests terminés (génération complète nécessite E2B + Gemini)")
    print("\n🎯 AMÉLIORATIONS PRINCIPALES :")
    print("   [OK] Gestion robuste des erreurs (KeyError, TypeError)")
    print("   [OK] Décodage des variables encodées")
    print("   [OK] Interprétations obligatoires après chaque élément visuel")
    print("   [OK] Références correctes (tableau vs graphique)")
